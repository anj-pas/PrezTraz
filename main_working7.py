...
from qgis.gui import QgsCrsSelectionWidget # 🚨 NOWY IMPORT DLA WIDŻETU CRS
...
# ==============================================================================
# 3. DIALOG WYBORU BAJTÓW WSPÓŁRZĘDNYCH I CRS 🚨 ZMIANA
# ==============================================================================
...
class SegyLoadDialog(QtWidgets.QDialog):
    def __init__(self, parent=None):
        ...
        # 🚨 Ustawienie domyślne na CRS projektu
    ...
    def get_selected_crs(self):
        # 🚨 Zwrócenie wybranego obiektu QgsCoordinateReferenceSystem
        return self.crs_widget.crs()

# ==============================================================================
# 4. CANVAS (WIZUALIZACJA SEJSMICZNA) - BEZ ZMIAN
# ==============================================================================

class SeismicImageCanvas(FigureCanvas):
    # ... (kod klasy SeismicImageCanvas jest bez zmian) ...
    def __init__(self, parent=None):
...
class SeismicPlugin:
    ...
    def __init__(self, iface):
        ...
        self.segy_crs = QgsProject.instance().crs() # 🚨 Domyślne CRS to CRS projektu
    ...
    def open_segy_file_dialog(self):
        ...
        self.x_byte = x_byte
        self.y_byte = y_byte
        # 🚨 ODZYSKANIE WYBRANEGO CRS
        self.segy_crs = dialog.get_selected_crs()
        
        if not self.segy_crs.isValid():
             iface.messageBar().pushCritical("Błąd CRS", "Wybrany CRS jest nieprawidłowy.")
             return
        ...
        if count > 0:
            iface.messageBar().pushMessage("Gotowe", 
                                          f"Pomyślnie załadowano {count} profili jako linie (CRS: {self.segy_crs.authid()}). Zaznacz linię, aby wyświetlić profil SEGY.", 
    ...
    def _manual_load_and_display(self, feature_id):
        ...
        try:
            # Modyfikacja mapowania przed otwarciem
    ...
    def add_feature_to_seismic_layer(self, fname):
        ...
        # W QGIS warstwa wektorowa może przechowywać dane w swoim własnym CRS, 
        # a QGIS automatycznie przetworzy je do CRS projektu. 
        # Ważne jest, aby geometrii nadać współrzędne w CRS warstwy.
    ...
    def _create_or_get_seismic_layer(self):
        ...
        # Sprawdzenie, czy istniejąca warstwa ma ten sam CRS co wybrany
        if self.seismic_layer:
             if self.seismic_layer.crs() == self.segy_crs:
                  return
             else:
                  # Jeśli CRS jest inny, usuń starą warstwę i utwórz nową. 
                  # Można by to obsłużyć lepiej, ale dla prostoty wtyczki: usuwamy.
                  QgsProject.instance().removeMapLayer(self.seismic_layer.id())
                  self.seismic_layer = None
                  self.selection_connection = None
                  self.layer_id_to_monitor = None
                  iface.messageBar().pushWarning("Ostrzeżenie", "Wykryto zmianę CRS. Poprzednia warstwa profili została usunięta, aby użyć nowego CRS.")


        # 🚨 Używamy wybranego CRS!
