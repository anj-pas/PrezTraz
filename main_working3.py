# main.py - Wersja Finalna z Zoomem Myszki, Strzałkami i Auto-wyświetlaniem ORAZ POPRAWIONYM IMPORTEM
...
from qgis.core import (
    ...
    QgsFeatureRequest # <--- POPRAWIONY IMPORT
)
import numpy as np
import segyio
import os
...
class SeismicImageCanvas(FigureCanvas):
    def __init__(self, parent=None):
        ...
        # Inicjalizacja funkcji interaktywnych
    ...
    def plot_seismic_image(self, data, dt_ms=2.0, clip_percent=100.0, cmap_name=DEFAULT_CMAP, 
        ...
        # 1. Rysowanie obrazu sejsmicznego
        ...
        self.ax.set_ylim(extent[3], extent[2]) # Odwrócona oś Y (czas)
        
        # Zmiana etykiety osi X (Poziomej)
    ...
    def _draw_orientation_and_title(self):
        ...
        # Unicode strzałki: \u2190 (lewo), \u2192 (prawo)
        
        # Góra-Lewo (Start profilu, Trace 1)
        ...
        # Góra-Prawo (Koniec profilu, Ostatni Trace)
    ...
    def on_scroll(self, event):
        ...
        # Obliczanie współczynnika zoomu (0.8 to zoom in, 1.2 to zoom out)
        if event.button == 'up':
            scale_factor = 0.8
        ...
        # Zoom Y
        new_ylim_top = ydata + cur_yrange * scale_factor * (cur_ylim[0] - ydata) / cur_yrange
        new_ylim_bottom = ydata - cur_yrange * scale_factor * (ydata - cur_ylim[1]) / cur_yrange
    ...
    def on_press(self, event):
        """Rozpoczęcie przesuwania (panning) za pomocą środkowego przycisku myszy."""
        if event.button == 2 and event.inaxes == self.ax: # Środkowy przycisk myszy
    ...
    def on_motion(self, event):
        """Przesuwanie widoku podczas ruchu myszy."""
        ...
        # Przesuwanie osi Y (uwaga na odwrócenie)
...
# ==============================================================================
# 5. SEISMIC PLUGIN - Zmodyfikowane (Auto-wyświetlanie)
# ==============================================================================
...
class SeismicPlugin:
    ...
    def open_segy_file_dialog(self):
        ...
        # --- LOGIKA AUTO-WYŚWIETLANIA ---
        # 1. Dokonujemy selekcji nowej cechy.
        self.seismic_layer.selectByIds([new_feature_id])
        
        # 2. Ręcznie wywołujemy ładowanie/wyświetlanie.
        self._manual_load_and_display(new_feature_id)
    ...
    def _manual_load_and_display(self, feature_id):
        """
        Ręczne wywołanie logiki ładowania SEGY dla nowo dodanej cechy.
        """
        # Użycie QgsFeatureRequest, który teraz jest zaimportowany
        ...
        if not feature_to_load:
            iface.messageBar().pushCritical("Błąd Wewnętrzny", "Nie można znaleźć nowo utworzonej cechy po ID.")
            return
        ...
        try:
            self.segy_handle = segyio.open(path, strict=False)
            ...
            iface.messageBar().pushMessage("Wizualizacja", f"Wyświetlam nowo załadowany profil: {profile_name}.", level=Qgis.MessageLevel.Info, duration=3) 
    ...
    def add_feature_to_seismic_layer(self):
        ...
        # Używamy tymczasowego uchwytu tylko do pobrania metadanych do cechy
        if self.segy_handle: self.segy_handle.close()
        temp_segy_handle = None 
        try:
             temp_segy_handle = segyio.open(self.fname, strict=False)
        except Exception as e:
        ...
        fields_to_add = []
        # Ponowne sprawdzenie pól (poprawiona logika dodawania, chociaż to już powinno być obsłużone przez _create_or_get_seismic_layer)
        if self.seismic_layer.fields().indexOf('start_x') == -1:
             fields_to_add.append(QgsField('start_x', QtCore.QVariant.Double))
             fields_to_add.append(QgsField('start_y', QtCore.QVariant.Double))
             fields_to_add.append(QgsField('end_x', QtCore.QVariant.Double))
             fields_to_add.append(QgsField('end_y', QtCore.QVariant.Double))
             if fields_to_add:
                 prov.addAttributes(fields_to_add)
                 self.seismic_layer.updateFields()
                 line_feature.setFields(self.seismic_layer.fields()) 
    ...
    def on_profile_selection(self, selected_ids, deselected_ids):
        ...
        # Używamy logiki z _manual_load_and_display, aby uniknąć duplikacji kodu
    ...
    def update_viewer_with_current_data(self):
        """Otwiera docka, jeśli nie jest otwarty, i aktualizuje obraz pełnymi danymi i metadanymi."""
    ...
    def open_seismic_viewer_dock(self):
        """Tworzy ZADOKOWANY dock, ustawiając domyślnie większą szerokość."""
        if self.viewer_dock is None:
            ...
            # --- Zwijalny Panel Kontrolny (Accordion/Toggle) ---
