# main.py - Wersja Finalna: Zestaw Poprawek
...
def calculate_seismic_attribute(data, attribute_name):
    """Oblicza wybrany atrybut sejsmiczny.
       Zabezpieczenie przed błędem 'str' object has no attribute 'shape'."""
    ...
    # Przetwarzanie kolumnami (śladami)
    n_samp, n_tr = data.shape
    output_data = np.zeros_like(data, dtype=np.float32)

    for i in range(n_tr):
        trace = data[:, i]
        # Sprawdzamy, czy ślad nie jest pusty/zerowy, aby uniknąć problemów z Hilbertem
        if np.all(trace == 0):
...
class SeismicImageCanvas(FigureCanvas):
    ...
    def plot_seismic_image(self, data, dt_ms=2.0, clip_percent=100.0, cmap_name=DEFAULT_CMAP, 
        ...
        # 🚨 KOREKTA GÓRA-DÓŁ: Odwracamy tablicę danych w pionie.
        # Odwracanie macierzy zapewnia, że pierwszy wiersz (minimalny czas)
        # jest wyświetlany na górze przy standardowym użyciu imshow (origin='upper').
        ...
        # Ustalanie zakresu wizualizacji (vmin/vmax)
        ...
        # Ponieważ dane zostały odwrócone, używamy standardowego (0, MaxTime) dla osi Y.
        self.im = self.ax.imshow(self.data, 
                               ...
                               vmin=vmin, 
                               vmax=vmax,
                               origin='upper') # Wiersz 0 jest na górze
        
        # Ustawienie pierwotnego zakresu widoku
        self.ax.set_xlim(0, n_tr)
        self.ax.set_ylim(self.times[-1], 0) # Wymuszamy, aby 0 było na górze, MaxTime na dole
    ...
    def on_scroll(self, event):
        ...
        cur_ylim = self.ax.get_ylim() # (MaxTime, 0)
        cur_xrange = cur_xlim[1] - cur_xlim[0]
        cur_yrange = cur_ylim[0] - cur_ylim[1] # MaxTime - 0
        ...
        # Logika zoomu dla odwróconej osi Y
        ...
        # Upewniamy się, że to jest (MaxTime, 0)
    ...
    def on_motion(self, event):
        ...
        # Oś Y jest odwrócona, więc przesunięcie musi być odwrócone.
        new_ylim_top = cur_ylim[1] - dy # Top (0)
        new_ylim_bottom = cur_ylim[0] - dy # Bottom (MaxTime)

        self.ax.set_xlim(new_xlim)
        self.ax.set_ylim(new_ylim_bottom, new_ylim_top) # Ustawienie (MaxTime, 0)
...
class SeismicPlugin:
    ...
    def _manual_load_and_display(self, feature_id):
        ...
        if path == self.fname and self.full_traces is not None:
            # Jeśli plik się nie zmienił, tylko aktualizujemy wyświetlanie atrybutu/klipowania
    ...
    def add_feature_to_seismic_layer(self, fname):
        # ... (bez zmian w tej funkcji - służy tylko do wczytywania metadanych i geometrii)
    ...
    def update_viewer_with_current_data(self):
        ...
        # Usunąłem argument 'data_to_plot', by wyeliminować błędy 'str' object has no attribute 'shape'

        if self.full_traces is None or self.viewer_canvas is None or self.viewer_dock is None:
        ...
        self.viewer_canvas.plot_seismic_image(self.full_traces, # Używamy pełnych danych
                                              dt_ms=self.dt_ms, 
    ...
    def open_seismic_viewer_dock(self):
        """Tworzy dock i dodaje panel Atrybutów i Ustawień."""
        if self.viewer_dock is None:
            ...
            attr_layout = QVBoxLayout(self.attributes_group)
            ...
            attr_layout.addWidget(QLabel("Seismic Attribute:"))
            attr_layout.addWidget(attribute_combo)
            
            def toggle_attr_group(checked):
                self.attributes_group.setMaximumHeight(100 if checked else 0)   

            toggle_button_attr.clicked.connect(toggle_attr_group)
            main_vbox.addWidget(toggle_button_attr)
            main_vbox.addWidget(self.attributes_group)

            
            # --- Zwijalny Panel Kontrolny (Visualization Settings) ---
            toggle_button_vis = QToolButton()
            toggle_button_vis.setText("☰ Visualization Settings")
            toggle_button_vis.setCheckable(True)
            toggle_button_vis.setChecked(False)
            ...
            def toggle_vis_group(checked):
                self.controls_group.setMaximumHeight(200 if checked else 0)   

            toggle_button_vis.clicked.connect(toggle_vis_group)
            
            main_vbox.addWidget(toggle_button_vis)
